...
export function createGitHubActivityItemsFromPayloads(payloads = {}, options = {}) {
  const importedAt = options.importedAt || new Date().toISOString();
  const workspaceId = options.workspaceId || null;
  const sourceConnectionId = options.sourceConnectionId || null;
  const repoOwner = options.repoOwner || 'unknown';
  const repoName = options.repoName || 'unknown';
  const repoKey = `${repoOwner}/${repoName}`;
  const items = [];

  for (const pull of Array.isArray(payloads.pulls) ? payloads.pulls : []) {
    ...
    items.push({
      ...
      source_id: String(pull.number || pull.id || ''),
      source_url: pull.html_url || null,
      title,
      body,
      author: pull.user?.login || 'github',
      occurred_at: pull.merged_at || pull.closed_at || pull.updated_at || pull.created_at || importedAt,
      imported_at: importedAt,
      tags: inferTags(`${title} ${body}`),
      product_area: inferProductArea(`${title} ${body}`),
      impact_hint: inferImpactHint(`${title} ${body}`),
      status: 'active',
      raw_payload: compactJson({ number: pull.number, state: pull.state, merged_at: pull.merged_at, title, url: pull.html_url }),
      dedupe_key: `github:${repoKey}:pr:${pull.number || pull.id}`,
    });
  }

  for (const commit of Array.isArray(payloads.commits) ? payloads.commits : []) {
    const message = commit.commit?.message || '';
    const title = message.split(/\r?\n/)[0] || `Commit ${String(commit.sha || '').slice(0, 7)}`;
    items.push({
      workspace_id: workspaceId,
      source_connection_id: sourceConnectionId,
      ...
      occurred_at: commit.commit?.author?.date || commit.commit?.committer?.date || importedAt,
      imported_at: importedAt,
      tags: inferTags(message || title),
      product_area: inferProductArea(message || title),
      impact_hint: inferImpactHint(message || title),
      status: 'active',
      raw_payload: compactJson({ sha: commit.sha, title, url: commit.html_url }),
      dedupe_key: `github:${repoKey}:commit:${commit.sha}`,
    });
  }

  for (const release of Array.isArray(payloads.releases) ? payloads.releases : []) {
    const title = release.name || release.tag_name || `Release ${release.id}`;
    const body = release.body || '';
    items.push({
      workspace_id: workspaceId,
      source_connection_id: sourceConnectionId,
      source_type: 'manual_release_note',
      source_id: String(release.id || release.tag_name || ''),
      source_url: release.html_url || null,
      title,
      body,
      author: release.author?.login || 'github',
      occurred_at: release.published_at || release.created_at || importedAt,
      imported_at: importedAt,
      tags: inferTags(`${title} ${body}`),
      product_area: inferProductArea(`${title} ${body}`),
      impact_hint: inferImpactHint(`${title} ${body}`),
      status: 'active',
      raw_payload: compactJson({ id: release.id, tag_name: release.tag_name, title, url: release.html_url }),
      dedupe_key: `github:${repoKey}:release:${release.id || release.tag_name}`,
    });
  }

  const seen = new Set();
  return items.filter((item) => {
    if (!item.dedupe_key || seen.has(item.dedupe_key)) return false;
...
function compactJson(value) {
  return JSON.stringify(value, null, 0).slice(0, 1800);
}
